...
// mobile-home-today-board.spec.ts (tranche 136, verified at 390px) at the
// 768px mobile->desktop transition zone, which tranche 136 never captured.
...
const PLAN_ROWS = [
  ...
  planRow({ plan_id: "P-T", plan_date: TODAY_ISO, item_name: "Margarita Base", status: "in_production" }),
];
...
test.describe("tablet WebKit (iPad Mini, 768px) — /home Today Board", () => {
  test("all three tabs render without a scroll trap", async ({ page }) => {
    ...
    await page.screenshot({ path: "test-results/tablet-today-board-screenshots/01-today.png", fullPage: true });

    await page.getByRole("tab", { name: "Yesterday" }).click();
    await expect(page.getByTestId("today-board-panel-yesterday")).toBeVisible();
    await page.waitForTimeout(200);
    await page.screenshot({ path: "test-results/tablet-today-board-screenshots/02-yesterday.png", fullPage: true });

    await page.getByRole("tab", { name: "Tomorrow" }).click();
    await expect(page.getByTestId("today-board-panel-tomorrow")).toBeVisible();
    await page.waitForTimeout(200);
    await page.screenshot({ path: "test-results/tablet-today-board-screenshots/03-tomorrow.png", fullPage: true });
