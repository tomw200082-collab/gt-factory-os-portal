...
- `docs/portal-os/tranches/039-dashboard-data-viz.md` — `/dashboard` first time-series band: Production activity (SVG area+line) + Stock movement flow (grouped bars), honest UOM-agnostic per-day counts aggregated client-side from existing proxies (no new backend); pure tested `_lib/trends.ts` helpers + `TrendChart` components; +dashboard-trends unit spec
