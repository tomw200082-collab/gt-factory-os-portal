...
// ---------------------------------------------------------------------------
// Portal proxy: POST /api/forecasts/open-draft
//   -> API POST /api/v1/mutations/forecasts/open-draft (G.4)
// ---------------------------------------------------------------------------
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    upstream = await fetch(
      `${API_BASE}/api/v1/mutations/forecasts/open-draft`,
      {
        method: "POST",
        headers: {
    ...
    return NextResponse.json(
      {
        error: "forecasts open-draft upstream unreachable",
