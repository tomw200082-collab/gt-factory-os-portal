...
// ---------------------------------------------------------------------------
// Portal proxy: POST /api/planning/runs/execute
//   -> API POST /api/v1/mutations/planning/run (Phase 7B write endpoint)
//
// Triggers a new planning run. Body carries idempotency_key +
// trigger_source + optional horizon params.
//
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    upstream = await fetch(
      `${API_BASE}/api/v1/mutations/planning/run`,
      {
        method: "POST",
        headers: {
    ...
    return NextResponse.json(
      {
        error: "planning run execute upstream unreachable",
