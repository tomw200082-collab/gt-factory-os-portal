...
export function QuickFixDrawer({
  ...
  const promote = useMutation({
    ...
    onSuccess: () => {
      ...
      // Tranche 144 — supplier_items is also read under the admin/components
  ...
  const updatePrice = useMutation({
    ...
    onSuccess: () => {
      ...
      // Tranche 144 — same cross-namespace sync as `promote` above.
      qc.invalidateQueries({ queryKey: ["api", "supplier-items"] });
      qc.invalidateQueries({ queryKey: ["admin", "supplier-items"] });
  ...
  const addLink = useMutation({
    mutationFn: async () => {
      ...
      if (bSetPrimary) {
        ...
        // Tranche 144 — same cross-namespace sync as `promote`/`updatePrice`.
        qc.invalidateQueries({ queryKey: ["api", "supplier-items"] });
        qc.invalidateQueries({ queryKey: ["admin", "supplier-items"] });
        onClose();
      }
